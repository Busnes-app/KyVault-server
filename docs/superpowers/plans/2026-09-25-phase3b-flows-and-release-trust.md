...
## Track B: KyForge release trust (planned only)

Track B lives outside this repository at `/home/yoshi/busness.app/kyforge-release-attestation-plan.md`, beside its hand-off, because KyVault's CI coordinate check rejects any Markdown file that names another product's image. Decision recorded 2026-09-25: plan only, do not execute. The route is keyless cosign against the public Sigstore instance with SLSA provenance and identity-bound verification in publish and promote; the trade-off is that the KyForge repository name and workflow path appear in the public transparency log.
