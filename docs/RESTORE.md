...
d=$(docker buildx imagetools inspect ghcr.io/busnes-app/kyvault-server:$sha --format '{{.Manifest.Digest}}') \
  ...
  && grep -qxF "KYVAULT_IMAGE=ghcr.io/busnes-app/kyvault-server@$d" .env
```
...
printf '%s\n' "$imgs" | grep -qxF "ghcr.io/busnes-app/kyvault-server@$d" || printf '%s\n' "$imgs" | grep -qxF 'kyvault-server:local' \
